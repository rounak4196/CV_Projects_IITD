...
test_questionsB_path = os.path.join(args.dataset, 'questions/CLEVR_testB_questions.json')
testB_image_directory = os.path.join(args.dataset, 'images/testB')
...
def evaluate(model:CLEVRDataset, loader):
    ...
    for i, idx in enumerate(chosen_correct):
        image, question, label = testB_dataset[idx]
        ...
        axes[i].axis('off')
        axes[i].set_title(f"Q: {question_text}\nPred: {testB_dataset.idx2answer[pred]}\nGT: {testB_dataset.idx2answer[label]}")
...
if __name__ == '__main__':
    ...
    if args.mode == 'inference':
        ...
        testB_dataset = CLEVRDataset(testB_image_directory,test_questionsB_path,)
        testB_loader = DataLoader(testB_dataset, batch_size=batch_size, num_workers=8)
        print("Test Data Loaded!!!!")

        evaluate(model,testB_loader)
